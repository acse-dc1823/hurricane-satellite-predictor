...
class StormDataset(Dataset):
    ...
    def __init__(self, root_dir, start_idx=0, end_idx=None, transform=None):
        ...
        self.data_path = sorted([file for file in os.listdir(root_dir)
                                 if file.endswith('.jpg')])[start_idx:end_idx]
    ...
    def __getitem__(self, idx):
        ...
        """
        img1 = Image.open(os.path.join(self.root_dir,
                                       self.data_path[idx])).convert('L')
        img2 = Image.open(os.path.join(self.root_dir,
                                       self.data_path[idx+1])).convert('L')
        img3 = Image.open(os.path.join(self.root_dir,
                                       self.data_path[idx+2])).convert('L')
        ...
        label1_path = os.path.join(
            self.root_dir,
            self.data_path[idx+2].replace('.jpg', '_label.json')
            )
        label3_path = os.path.join(
            self.root_dir,
            self.data_path[idx+2].replace('.jpg', '_label.json')
            )
        ...
        stacked_img = torch.cat([float(label1)*eye_tensor,
                                 img1, img2, img3], dim=0)
...
def get_train_val_loader(dataset, train_size=0.8):
    ...
    and create DataLoader instances.

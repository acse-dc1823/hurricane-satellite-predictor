...
def show_speed_distribution(path):
    ...
    ----------
    path : str
        Root path containing folders for each hurricane
        with associated label JSON files.
    ...
    for i in range(len(dic_list)):
        wind_speed = []
        label_list = [file for file in os.listdir(dic_list[i])
...
def show_one_image_per_storm(root_dir, rows=3, cols=10):
    """
    This function displays the first image from each storm.
    The images are displayed in a grid layout
    specified by the rows and columns parameters.

    Parameters:
    - root_dir (str):
        The root directory containing subdirectories for each storm,
        where each subdirectory contains images for that storm.
    - rows (int):
        The number of rows in the grid of images to display.
    - cols (int):
        The number of columns in the grid of images to display.
    ...
    storms = [d for d in os.listdir(root_dir)
    ...
    for i, storm in enumerate(storms):
        # Stop if the maximum number of images for the grid is reached
        if i >= rows * cols:
            break

        storm_dir = os.path.join(root_dir, storm)
        image_files = [file for file in sorted(os.listdir(storm_dir))
...
def show_samples(dataset, storm_id):
    """
    Display the first 100 images and their associated features
    and labels for a specific storm.

    Parameters:
    - dataset:
        The dataset object containing the storm images, features, and labels.
    - storm_id:
        A string representing the unique identifier of the storm.
    ...
    The function will filter out the files from the dataset
    corresponding to the specified storm_id,
    plot them in a grid, and show associated features like the storm ID,
    relative time from the start of the storm sequence,
    and the wind speed as the label.
    """
    ...
    fig, axs = plt.subplots(rows, cols, figsize=(25, 2.5 * rows))
